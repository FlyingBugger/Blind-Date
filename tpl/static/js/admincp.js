...
$(function(){
	...
	$("[f='creditget']").bind("click", function(){
		...
		$.ajax({
			type: "POST",
			url: _ROOT_PATH+"index.php?c=credit&a=get",
			cache: false,
...
$(function(){
    //审核通过
    $('[f="partybm_pass"]').click(function(){
        $_id = $(this).attr("data-id");
        $url = _CPFILE+"?c=partybm&a=pass&id="+$_id+"&r="+get_rndnum(6)+"&keepThis=true&TB_iframe=true&width=700&height=270";
        tb_show("活动报名审核通过", $url, false);
    });

    //取消报名
    $('[f="partybm_cancel"]').click(function(){
        $_id = $(this).attr("data-id");
        $url = _CPFILE+"?c=partybm&a=cancel&id="+$_id+"&r="+get_rndnum(6)+"&keepThis=true&TB_iframe=true&width=700&height=335";
        tb_show("取消活动报名", $url, false);
    });
});
 ...
 * @date:: 2017.12.07
 */
function oelove_varpop_creditview(title, userid) {
    var filename = get_curl();
    var url = filename + "?c=viewrp&userid="+userid+"&fromtype=jdbox&r="+get_rndnum(6);
    url = url + "&keepThis=true&TB_iframe=true&width=750&height=480";
    tb_show(title, url, false);
}