...
echo  "<style>body{font-size:12px;line-height:25px;}</style>".
"<body>".$error.
"</body>";
die();
}
...
public static function callBackJson($response=0,$result='') {
echo json_encode(array("response"=>$response,"result"=>$result));
die();
}
public static function getValidID($rqstype = "") {
$_id = XRequest::getInt("id");
if ($_id <1) {
if ($rqstype == "ajax") {
self::callBackJson(0,"ID参数错误");
}
else {
XHandle::halt("ID参数错误","",1);
}
}
return $_id;
}
public static function getValidArrayID($rqstype = "") {
$_ids = XRequest::getArray("id");
if (empty($_ids)) {
if ($rqstype == "ajax") {
self::callBackJson(0,"IDs参数错误");
}
else {
XHandle::halt("IDs参数错误","",1);
}
}
return $_ids;
}
